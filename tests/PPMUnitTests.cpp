...
BOOST_AUTO_TEST_CASE(ReadOneSymbolImage) {
    std::string file_path = ppm_path + "OneSymbolImage.ppm";
    std::string file_format = "P6";
    size_t height = 1;
    size_t width = 1;
    size_t max_value = 255;
    PPM::Pixel original_pixel('J', 'I', 'M');

    BinaryPPM image(file_path);
    ...
    PPM::Pixel result_pixel = image.get_pixel(0, 0);
    BOOST_CHECK_EQUAL(original_pixel, result_pixel);
}

BOOST_DATA_TEST_CASE(LoadImage, PPM_BINARY_FILE_NAMES, file_name) {
...
BOOST_DATA_TEST_CASE(SaveImage, PPM_BINARY_FILE_NAMES, file_name) {
    ...
    BinaryPPM image(result_file);

    image.load();
    image.save();

    BinaryPPM new_image(result_file);
    new_image.load();
...
BOOST_AUTO_TEST_SUITE(TextPPMTests)
...
BOOST_AUTO_TEST_CASE(ReadOnePixelImage) {
    ...
    size_t width = 1;
    size_t max_value = 255;
    PPM::Pixel original_pixel(24, 54, 48);
...
BOOST_AUTO_TEST_SUITE_END(/*TestPPMTests*/)
